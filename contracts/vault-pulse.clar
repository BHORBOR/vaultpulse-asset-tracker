...
(define-map assets 
  { owner: principal, asset-id: (string-ascii 36) } 
  {
...
(define-map asset-history
  { owner: principal, asset-id: (string-ascii 36), timestamp: uint }
  {
    value: uint
  }
)
...
(define-map user-categories
  { owner: principal, category-id: (string-ascii 50) }
  {
    name: (string-utf8 100),
...
(define-map authorized-viewers
  { vault-owner: principal, viewer: principal }
  {
    authorized-at: uint,
...
(define-map monitoring-thresholds
  { owner: principal, asset-id: (string-ascii 36), threshold-id: (string-ascii 36) }
  {
...
(define-private (is-authorized (vault-owner principal))
  (or 
    ...
    (let ((viewer-info (get-authorized-viewer vault-owner tx-sender)))
      (and 
        (is-some viewer-info)
        (let ((expiration (get expiration (unwrap! viewer-info false))))
          (or 
            (is-none expiration)
            (> (unwrap! expiration u0) block-height)
          )
        )
      )
    )
...
(define-private (get-authorized-viewer (vault-owner principal) (viewer principal))
  (map-get? authorized-viewers { vault-owner: vault-owner, viewer: viewer })
)

;; Check if asset exists
(define-private (asset-exists (owner principal) (asset-id (string-ascii 36)))
  (is-some (map-get? assets { owner: owner, asset-id: asset-id }))
)

;; Record a new valuation in the asset history
(define-private (record-valuation (owner principal) (asset-id (string-ascii 36)) (value uint))
  (let ((current-time block-height))
    (map-set asset-history 
      { owner: owner, asset-id: asset-id, timestamp: current-time }
      { value: value }
    )
    current-time
  )
...
(define-private (ensure-vault-exists (owner principal) (name (string-utf8 100)))
...
(define-private (can-view-asset (asset-owner principal) (asset-id (string-ascii 36)))
  (let ((asset-info (map-get? assets { owner: asset-owner, asset-id: asset-id })))
    (or
...
;; Get asset details
(define-read-only (get-asset (owner principal) (asset-id (string-ascii 36)))
  (begin
    (asserts! (can-view-asset owner asset-id) ERR-NOT-AUTHORIZED)
    (map-get? assets { owner: owner, asset-id: asset-id })
  )
)

;; Get asset valuation at a specific time
(define-read-only (get-asset-valuation-at (owner principal) (asset-id (string-ascii 36)) (timestamp uint))
  (begin
    (asserts! (can-view-asset owner asset-id) ERR-NOT-AUTHORIZED)
    (map-get? asset-history { owner: owner, asset-id: asset-id, timestamp: timestamp })
  )
)

;; Get vault information
(define-read-only (get-vault-info (owner principal))
  (let ((vault-info (map-get? vaults { owner: owner })))
    (if (or (is-eq tx-sender owner) 
            (is-authorized owner)
            (and (is-some vault-info) (get public-view (unwrap! vault-info false))))
      vault-info
      ERR-NOT-AUTHORIZED
    )
  )
...
(define-read-only (get-threshold (owner principal) (asset-id (string-ascii 36)) (threshold-id (string-ascii 36)))
  (begin
    (asserts! (can-view-asset owner asset-id) ERR-NOT-AUTHORIZED)
    (let ((threshold (map-get? monitoring-thresholds 
                      { owner: owner, asset-id: asset-id, threshold-id: threshold-id })))
...
(define-read-only (check-thresholds (owner principal) (asset-id (string-ascii 36)))
  (begin
    (asserts! (can-view-asset owner asset-id) ERR-NOT-AUTHORIZED)
    (let ((asset-info (map-get? assets { owner: owner, asset-id: asset-id })))
...
(define-public (register-asset 
  ...
  (let ((owner tx-sender))
    ...
    (ensure-vault-exists owner "My Vault")
    ...
    (map-set vaults
      { owner: owner }
      (merge (unwrap! (map-get? vaults { owner: owner }) ERR-VAULT-NOT-FOUND)
             { last-updated: block-height })
    )
...
(define-public (update-asset-value (asset-id (string-ascii 36)) (new-value uint))
  (let ((owner tx-sender))
    ;; Check that asset exists
    (asserts! (asset-exists owner asset-id) ERR-ASSET-NOT-FOUND)
    
    ;; Get current asset info
    (let ((asset-info (unwrap! (map-get? assets { owner: owner, asset-id: asset-id }) ERR-ASSET-NOT-FOUND)))
...
(define-public (delete-asset (asset-id (string-ascii 36)))
  (let ((owner tx-sender))
    ...
    (map-delete assets { owner: owner, asset-id: asset-id })
...
(define-public (set-vault-info
  (vault-name (string-utf8 100))
  (public-view bool))
  
  (let ((owner tx-sender))
    ;; Check if vault exists, create if it doesn't
    (ensure-vault-exists owner vault-name)
    ...
    (map-set vaults
      { owner: owner }
      {
        ...
        creation-date: (get creation-date (unwrap! (map-get? vaults { owner: owner }) block-height)),
        public-view: public-view,
        last-updated: block-height
      }
    )
...
(define-public (authorize-viewer
  ...
  (let ((owner tx-sender))
    ...
    (map-set authorized-viewers
      { vault-owner: owner, viewer: viewer }
      {
...
(define-public (revoke-viewer (viewer principal))
  (let ((owner tx-sender))
    ...
    (map-delete authorized-viewers { vault-owner: owner, viewer: viewer })
...
(define-public (delete-threshold
  ...
  (let ((owner tx-sender))
    ...
    (asserts! (is-some (map-get? monitoring-thresholds 
                       { owner: owner, asset-id: asset-id, threshold-id: threshold-id }))
             ERR-THRESHOLD-NOT-FOUND)
